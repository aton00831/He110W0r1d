...
object ColumnsAndExpressions extends App {
  ...
  // Columns
  val firstColumn = carsDF.col("Name") // No data inside, using in select to get data

  // selecting (projecting)
  val carNamesDF = carsDF.select(firstColumn) // new dataframe becuase df is immutable
  ...
  // careful with column names (use ` to escape in expr expression)
  carsWithColumnRenamed.selectExpr("`Weight in pounds`")
  // remove a column
  carsWithColumnRenamed.drop("Cylinders", "Displacement")

  // filtering
  val europeanCarsDF = carsDF.filter(col("Origin") =!= "USA")  // != conflict with scala operator, using =!=
  val europeanCarsDF2 = carsDF.where(col("Origin") =!= "USA")  // exact the same thing
