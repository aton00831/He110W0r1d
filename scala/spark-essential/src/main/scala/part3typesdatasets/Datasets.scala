...
(Dataframe = Distribute collection of Untyped ROW)

Most useful when
- we want to maintain type information
- we want clean concise code ( In production application)
- out filters/ transformation are hard to express in DF or SQL
...
object Datasets extends App {
  ...
  // now can run : numberDS.filter(_ < 100)
  ...
  // implicit val carEncode = Encoders.product[Car]
  ...
  val guitarPlayerBandsDS: Dataset[(GuitarPlayer, Band)] = guitarPlayersDS.joinWith(
    bandsDS, guitarPlayersDS.col("band") === bandsDS.col("id"), "inner")
  // Error because inferSchema = True
  // output _1, _2, use .withColumnRenamed
  /**
