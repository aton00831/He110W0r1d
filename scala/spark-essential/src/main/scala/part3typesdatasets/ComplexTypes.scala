...
object ComplexTypes extends App {
  ...
  moviesWithReleaseDates.select("*").where(col("Actual_Release").isNull) // different format -> null
  ...
  // 1 - parse the DF multiple times, then union the small DFs (or just ignore those cases ...)
